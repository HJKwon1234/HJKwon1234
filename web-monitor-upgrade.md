...
이 도구는 DDoS 모의훈련 현장에서 다음과 같은 흐름으로 사용됩니다.
**업그레이드 시 이 워크플로우를 절대 방해하지 않아야 합니다.**

### 훈련 흐름

```
[훈련 전] sites.json에 모든 타겟 URI 사전 정의
         (훈련 중 수정할 시간적 여유 없음)
    ↓
[1차 공격 준비] python 실행 → sites.json에서 1차 대상 선택 → 스탠바이
    ↓
[1차 공격 시작] 공격 시작 사인 → 엔터 → 즉시 모니터링 시작
    ↓
[1차 공격 종료 직전] Ctrl+C → 산출물 자동 저장
    ↓
[2차 공격 준비] python 재실행 → sites.json에서 2차 대상 선택 → 스탠바이
              (1차 산출물 수집과 2차 준비를 동시 진행)
    ↓
[2차 공격 시작] 공격 시작 사인 → 엔터 → 즉시 모니터링 시작
    ↓
    ... (반복)
```
...
| 설계 | 이유 |
|------|------|
| **sites.json 분리** | 훈련 중 대상 서버 정보 수정 불가 → 사전 정의 필수 |
| **엔터 키 스탠바이** | 파이썬 실행 시간이 길어질 수 있음 → 미리 실행 후 대기 |
| **Ctrl+C 즉시 종료 + 자동 저장** | 1차 종료 → 산출물 수집 → 2차 준비를 빠르게 전환 |
| **다중 사이트 선택** | 같은 json에서 공격 차수별로 다른 URI 선택 |

### 업그레이드 금지 사항

- 시작 시 입력 프롬프트를 과도하게 추가하지 않을 것 (빠른 실행 필요)
- sites.json 구조를 변경하지 않을 것 (기존 파일 그대로 사용 가능해야 함)
- 스탠바이 → 엔터 → 즉시 시작 흐름을 변경하지 않을 것
- Ctrl+C 후 산출물 저장 로직을 변경하지 않을 것
